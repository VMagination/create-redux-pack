...
const createSelector$1 = (reducerOrSource, keyOrFormat) => {
  ...
  return createSelector(source, selection);
};
...
const resetAction = Object.assign(createAction(RESET_ACTION_TYPE), {
...
const wrapSelector = (prevSelector, key, format = state => state) => new Proxy(createSelector(prevSelector, state => format((state || {})[key])), {
  get: (target, p) => {
...
const addMappedSelectors = (obj, payloadMap, name, prevSelector, prefix = '') => {
  ...
  keys.forEach(key => {
    ...
    const sourceSelector = createSelector(prevSelector, state => {
      const getReadable = (() => {
        let lastResult = null;
        return target => {
...
const createReduxPack = Object.assign(infoRaw => {
  ...
  _generator: {
    actions: ({
      ...
      run: createAction$1(createReduxPack.getRunName(name)),
      success: createAction$1(createReduxPack.getSuccessName(name), formatPayload),
      fail: createAction$1(createReduxPack.getFailName(name))
    }),
    actionNames: ({
      name
    }) => ({
    ...
    selectors: ({
      ...
      return {
        isLoading: createSelector(getReducerState, state => state[createReduxPack.getLoadingName(name)]),
        result: createSelector(getReducerState, state => state[createReduxPack.getResultName(name)]),
        error: createSelector(getReducerState, state => state[createReduxPack.getErrorName(name)]),
