...
const resetAction = Object.assign(toolkit.createAction(RESET_ACTION_TYPE), {
...
const wrapSelector = (prevSelector, key, format = state => state) => new Proxy(reselect.createSelector(prevSelector, state => format((state || {})[key])), {
  get: (target, p) => {
...
const addMappedSelectors = (obj, payloadMap, name, prevSelector, prefix = '') => {
  ...
  keys.forEach(key => {
    ...
    const sourceSelector = reselect.createSelector(prevSelector, state => {
      const getReadable = (() => {
        let lastResult = null;
        return target => {
...
const createReduxPack = Object.assign(infoRaw => {
  ...
  _generator: {
    ...
    selectors: ({
      ...
      return {
        isLoading: reselect.createSelector(getReducerState, state => state[createReduxPack.getLoadingName(name)]),
        result: reselect.createSelector(getReducerState, state => state[createReduxPack.getResultName(name)]),
        error: reselect.createSelector(getReducerState, state => state[createReduxPack.getErrorName(name)]),
