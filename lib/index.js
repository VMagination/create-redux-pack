...
var getIt = function getIt() {
  ...
  var find = function find(regexp) {
    return path.split(regexp).filter(Boolean).reduce(function (res, key) {
...
var addMappedPayloadToState = function addMappedPayloadToState(obj, payloadMap, name, payload, state) {
  ...
  keys.forEach(function (key) {
    ...
    var nextPrefix = "".concat(prefix).concat(!hasCRPackName(key) ? key : key.replace(/ of .+/, ''), ".");
  ...
  return new Proxy(reselect.createSelector(prevSelector, function (state) {
    ...
    get: function get(target, p) {
      if (!target[selectorContent]) {
        target[selectorContent] = defineProperty({}, p, wrapSelector(target, p));
      } else {
...
var wasd = createReduxPack({
  name: 'PackWithGenerator',
  reducerName: 'asd' + 4,
  resultInitial: []
}).withGenerator(mergeGenerators({
  initialState: function initialState(_ref17) {
    var name = _ref17.name;
    return defineProperty({}, name + 'Flag', false);
  },
  stateNames: function stateNames(_ref19) {
    var name = _ref19.name;
    return {
      flag: name + 'Flag'
    };
  },
  actionNames: function actionNames(_ref20) {
    var name = _ref20.name;
    return {
      reset: name + 'Reset'
    };
  },
  actions: function actions(_ref21) {
  ...
  reducer: function reducer(_ref22) {
    ...
    return _ref25 = {}, defineProperty(_ref25, createReduxPack.getRunName(name), createReducerCase(function () {
      var _ref23;

      return _ref23 = {}, defineProperty(_ref23, createReduxPack.getLoadingName(name), false), defineProperty(_ref23, "somethingCool", 'right here'), _ref23;
    })), defineProperty(_ref25, name + 'Reset', createReducerCase(function () {
      var _ref24;

      return _ref24 = {}, defineProperty(_ref24, createReduxPack.getResultName(name), []), defineProperty(_ref24, name + 'Flag', true), _ref24;
    })), _ref25;
  },
  selectors: function selectors(_ref26) {
    var reducerName = _ref26.reducerName,
        name = _ref26.name;
    return {
  ...
  reducer: function reducer(_ref27) {
    var name = _ref27.name;
    return defineProperty({}, createReduxPack.getRunName(name), function () {
      return {
