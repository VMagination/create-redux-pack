...
const createReduxPack = Object.assign(infoRaw => {
  ...
  generator: {
    ...
    selectors: ({
      ...
      return {
        isLoading: reselect.createSelector(getReducerState, state => state[getLoadingName(name)]),
        result: reselect.createSelector(getReducerState, state => state[getResultName(name)]),
        error: reselect.createSelector(getReducerState, state => state[getErrorName(name)]),
        ...Object.keys(payloadMap).reduce((accum, key) => {
          const format = payloadMap[key]?.formatSelector || (state => state);

          return { ...accum,
            [key]: reselect.createSelector(getReducerState, state => format(state[getKeyName(name, `${key}`)]))
          };
        }, {})
      };
    },
